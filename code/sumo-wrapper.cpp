...
// Base name (including path) for the files generated by this process for
// this instance. Prefixed with the PID so that several sumo-wrapper
// processes can run concurrently on the same instance without clobbering
// each other's files.
string runPrefix(const cInstance &c);

// Removes the temporary files generated by this process for this instance
void removeTempFiles(const cInstance &c);

// Build XML additional file with new tlLogic tags from TL configuration
void buildXMLfile(const cInstance &c, const vector<unsigned> &tl_times, time_t t);
...
string buildCommand(const cInstance &c, time_t t);
...
string runPrefix(const cInstance &c)
{
	return c.getPath() + ::to_string((unsigned long)getpid()) + "-" + c.getName();
}

void removeTempFiles(const cInstance &c)
{
	string prefix = runPrefix(c);
	remove((prefix + ".add.xml").c_str());
	remove((prefix + "-summary.xml").c_str());
	remove((prefix + "-vehicles.xml").c_str());
	remove((prefix + "-tripinfo.xml").c_str());
}

void buildXMLfile(const cInstance &c, const vector<unsigned> &tl_times, time_t t)
{
	string xmlfile = runPrefix(c) + ".add.xml";
...
{
	string cmd = "sumo -W ";
	string name1 = c.getPath() + c.getName(); // Required instance files (shared, read-only)
	string name2 = runPrefix(c); // files generated by this process, PID-prefixed to avoid clashes with concurrent runs
...
//	cmd += "--emission-output " + name2 + "-emissions.xml "; // Emissions result
...
void analyzeTripInfo(const cInstance &c, time_t t, tStatistics &s)
{
	string filename = runPrefix(c) + "-tripinfo.xml";
...
void analyzeSummary(const cInstance &c, time_t t, tStatistics &s)
{
	string filename = runPrefix(c) + "-summary.xml";
...
void analyzeEmissions(const cInstance &c, time_t t, tStatistics &s)
{
	string filename = runPrefix(c) + "-emissions.xml";
	string line;
	ifstream fin(filename.c_str());
	map<string, string> m;
	int position;
